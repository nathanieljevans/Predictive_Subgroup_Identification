...
<<<<<<< HEAD
    ...
    def recursive_gen(self, subgroup): 
        # exit test 
        if (len(subgroup.subgroup_ids)  == self.num_patients or self.index > 100): 
...
#            wr.writerow(list(self.response_vector))
            np.savetxt('C:\\Users\\Nate\\Desktop\\subgroup_csv\\subgroup_treatment.csv', self.response_vector, delimiter=',')
            return 0 
